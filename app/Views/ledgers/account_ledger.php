  ...
  <div class="row">
    <div class="col-12">
      <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">Account Ledger: <?= esc($account['account_name']) ?></h4>
        <div class="page-title-right">
          <ol class="breadcrumb m-0">
            <li class="breadcrumb-item"><a href="<?= base_url('ledgers/accounts') ?>">Accounts</a></li>
        ...
        <div class="col-auto ms-auto">
          <div class="btn-group">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              <li><a class="dropdown-item" href="<?= base_url('ledgers/export/account/' . $account['id']) ?>?format=csv&from_date=<?= $fromDate ?>&to_date=<?= $toDate ?>">CSV (Excel)</a></li>
              <li><a class="dropdown-item" href="<?= base_url('ledgers/export/account/' . $account['id']) ?>?format=pdf&from_date=<?= $fromDate ?>&to_date=<?= $toDate ?>" target="_blank">Print / PDF</a></li>
            </ul>
          </div>
        </div>
      </form>
              ...
              <tbody>
                ...
                <?php else: ?>
                  <?php foreach ($entries as $entry): ?>
                    <?php
                    // Ensure correct calculation based on stored data
                    // The stored balance_after is historically correct at time of insertion
                    // BUT if we filter by date, we might want to recalculate running balance from opening?
                    // YES, standard practice: Recalculate visually based on displayed rows + opening.

    ...
    var table = $('#cashCustomersTable').DataTable({
      ...
      columns: [{
        ...
        },
        {
          ...
          orderable: false,
          render: function(data, type, row) {
            let canView = <?= json_encode($action_flags['view'] ?? false) ?>;
            let canEdit = <?= json_encode($action_flags['edit'] ?? false) ?>;
            let canDelete = <?= json_encode($action_flags['delete'] ?? false) ?>;
            ...
            if (canView) {
              html += `<a class="dropdown-item" href="<?= base_url('customers/cash-customers') ?>/${data}"><i class="ri-eye-line me-1"></i> View</a>`;
            }
            if (canEdit) {
              html += `<a class="dropdown-item" href="<?= base_url('customers/cash-customers') ?>/${data}/edit"><i class="ri-pencil-line me-1"></i> Edit</a>`;
            }
            if (canDelete) {
              html += `<a class="dropdown-item text-danger delete-record" href="javascript:void(0);" data-id="${data}"><i class="ri-delete-bin-line me-1"></i> Delete</a>`;
            }

...
$lines          = $challan['lines']           ?? [];
$created_at     = $challan['created_at'] ?? null;
...
$dateStr = $challanDate ? date('d/m/Y', strtotime($challanDate)) : date('d/m/Y');
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  <meta charset="UTF-8">
  <title>Cash Memo <?= esc($challanNumber) ?></title>
  <style>
    * {
      margin: 0;
      padding: 0;
  ...
  <div class="receipt-container">
    ...
    <table class="meta-table">
      <tr>
        ...
        <td><strong>Bill No:</strong> <?= str_pad(esc($challanNumber), 5, '0', STR_PAD_LEFT) ?></td>
      </tr>
      <tr>
    ...
    <table class="items-table">
      ...
      <tfoot>
        <tr>
          <td colspan="4" style="border-top: 1px solid #555;"></td>
        </tr>
        <tr>
          <td colspan="3" class="text-right"><strong>Total Amount:</strong></td>
          <td class="text-right"><strong>₹ <?= number_format((float)($challan['total_amount'] ?? 0), 2) ?></strong></td>
        </tr>
      </tfoot>
    </table>
  </div>

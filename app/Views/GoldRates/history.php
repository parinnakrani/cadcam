            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    foreach($history as $h) {
                        $date = $h['rate_date'];
                        if(!isset($grouped[$date])) $grouped[$date] = ['22K'=>'-', '24K'=>'-', 'Silver'=>'-', 'user'=>'-'];
                        $grouped[$date][$h['metal_type']] = $h['rate_per_gram'];
                        $grouped[$date]['user'] = $h['created_by']; // Should resolve user name ideally
                    }
                    ?>
                    <?php if(empty($grouped)): ?>
                        <tr><td colspan="5" class="text-center">No records found</td></tr>
                    <?php else: ?>
                        <?php foreach($grouped as $date => $rates): ?>
                        <tr>
                            <td><?= date('d M Y', strtotime($date)) ?></td>
                            <td><?= $rates['22K'] ?></td>
                            <td><?= $rates['24K'] ?></td>
                            <td><?= $rates['Silver'] ?></td>
                            <td>User #<?= $rates['user'] ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
...
<script>
    ...
    // Prepare datasets
    const datasets = [];
    const colors = {'22K': '#ffc107', '24K': '#ff9f43', 'Silver': '#8592a3'};
    
    Object.keys(chartData).forEach(metal => {
        if(chartData[metal].length > 0) {
            datasets.push({
                label: metal,
                data: chartData[metal].map(d => ({x: d.date, y: d.rate})),

...
class AuthService
{
  ...
  public function login(string $username, string $password, bool $remember = false): ?array
  {
    ...
    if (!$user) {
      $this->auditService->log('Auth', 'login', 'User', 0, null, ['username' => $username, 'status' => 'failed']);
    ...
    if ($user['failed_login_attempts'] >= 5) {
      $this->auditService->log(
        ...
        $user['id'],
        null,
        ['user_id' => $user['id'], 'attempts' => $user['failed_login_attempts'], 'status' => 'locked']
      );
    ...
    if ($user['employment_status'] !== 'Active') {
      $this->auditService->log(
        ...
        $user['id'],
        null,
        ['user_id' => $user['id'], 'status' => $user['employment_status'], 'reason' => 'inactive']
      );
  ...
  public function logout(): bool
  {
    ...
    if ($currentUser) {
      $this->auditService->log('Auth', 'logout', 'User', $currentUser['id']);
    }
  ...
  private function handleFailedLogin(int $userId): void
  {
    ...
    $this->auditService->log('Auth', 'access_denied', 'User', $userId, null, ['reason' => 'Invalid password']);
  }
  ...
  public function attemptAutoLogin(): bool
  {
    ...
    if (
      $user['remember_token'] === $cookieToken &&
